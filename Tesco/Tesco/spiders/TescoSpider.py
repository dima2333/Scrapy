...
class TescospiderSpider(scrapy.Spider):
    ...
    def parse_review(self, response):
        ...
        for i in SET_SELECTOR:
            item['product_url'] = i.xpath('.//ancestor::body[@id="data-attributes"]/preceding-sibling::head/link[@rel="canonical"]/@href').get(default=""),
            item['product_id'] = i.xpath('.//ancestor::body[@id="data-attributes"]/preceding-sibling::head/link[@rel="canonical"]/@href').get(default=0)[-9:],
            item['image_url'] = i.xpath('.//div[@class="product-details-tile__main"]//img/@src').get(default=""),
            item['product_title'] = i.xpath('.//h1[@class="product-details-tile__title"]/text()').get(default=""),
            item['category'] = i.xpath('.//div[@class="grocery-product__related-links"]//span[contains(@class,"button")]/text()').get(default="")[9:],
            item['price'] = i.xpath('.//div[contains(@class,"control-wrapper")]//span[@data-auto="price-value"][1]/text()').get(default="0.0"),
            item['product_d'] = i.xpath('.//div[@class="product-blocks"]/div[@id="product-description"]/ul/li/text()').getall(),
            item['pack_size'] = i.xpath('.//div[@class="product-blocks"]//li[contains(text(),"Pack")]/text()').get(default="")[11:],
            ...
            for index in range(0, count_nodes_next):
                ...
                itemn['product_url_next'] = SET_SELECTOR.xpath('//div[@class="recommender__wrapper"]/div[@class="product-tile-wrapper"]/descendant::a[@data-auto="product-tile--title"]/@href').getall()[index],
                itemn['product_title_next'] = SET_SELECTOR.xpath('//div[@class="recommender__wrapper"]/div[@class="product-tile-wrapper"]//a[@data-auto="product-tile--title"]/text()').getall()[index],
                itemn['image_url_next'] = SET_SELECTOR.xpath('//div[@class="recommender__wrapper"]/div[@class="product-tile-wrapper"]//img/@src').getall()[index],
            ...
            self.item_rlist = []
            max_page = math.ceil(prod_review_info['total'] / prod_review_info['count']) + 1
